...
@section('content')
    ...
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title text-bold">Edit Banner</h3>
            <div class="heading-elements">
                <a href="{{ url('admin/home-page-banners') }}" class="btn btn-primary">List Banner</a>
            </div>
        </div>
        <div class="panel-body">
            {{ Form::open(array('method' => 'PATCH','route' => ['home-page-banners.update',$homePageBanners->id], 'class'=>'form-vertical', 'id' => 'portfolio_form', 'files' => true )) }}

            <div class="form-group">
                <label class="control-label text-bold" for="title">Title <span class="required-fields">*</span></label>
                <input type="text" name="title" id="title" class="form-control" required data-msg-required="Please enter banner title." value="{{$homePageBanners->title ?? ''}}">
                <span class="validation-errors"></span>
            </div>

            @if($homePageBanners->image_link)
                <div class="form-group">
                    <label class="control-label text-bold" for="image_link">Image</label>
                    <input type="file" name="image_link" id="image_link" class="form-control" data-msg-required="Please select a banner image file." accept="image/*">
                    <span class="validation-errors"></span>
                </div>
                <div class="form-group">
                    <img src="{{ URL::asset($homePageBanners->image_link) }}" class="img-thumbnail" width="100">
                </div>
            @else
                <div class="form-group">

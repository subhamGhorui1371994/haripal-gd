...
@section('content')
    ...
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title text-bold">Edit Event</h3>
            <div class="heading-elements">
                <a href="{{ url('admin/event') }}" class="btn btn-primary">List Event</a>
            </div>
        </div>
        <div class="panel-body">
            {{ Form::open(array('method' => 'PATCH','route' => ['event.update',$event->id], 'class'=>'form-vertical', 'id' => 'portfolio_form', 'files' => true )) }}

            <div class="form-group">
                <label class="control-label text-bold" for="date">Date <span class="required-fields">*</span></label>
                <input type="text" name="date" id="date" class="form-control" required data-msg-required="Please select event date." value="{{$event->date ? date('d-m-Y', strtotime($event->date)) : ''}}">
                <span class="validation-errors"></span>
            </div>

            <div class="form-group">
                <label class="control-label text-bold" for="title">Title <span class="required-fields">*</span></label>
                <input type="text" name="title" id="title" class="form-control" required data-msg-required="Please enter event title." value="{{$event->title ?? ''}}">
                <span class="validation-errors"></span>
            </div>

            <div class="form-group">
                <label class="control-label text-bold" for="description">Description <span class="required-fields">*</span></label>
                <textarea class="form-control" rows="5" id="description" name="description" required data-msg-required="Please enter event description.">{{$event->description ?? ''}}</textarea>
                <span class="validation-errors"></span>
            </div>

            <div class="form-group">
                <label class="control-label text-bold" for="event_file">Image</label>
                <input type="file" name="event_file" id="event_file" class="form-control" data-msg-required="Please select a event image." accept="image/*">
                <span class="validation-errors"></span>
            </div>

            @if($event->image)
                <div class="form-group">
                    <img src="{{ URL::asset($event->image) }}" class="img-thumbnail" width="100">
                </div>
            @endif

            <div class="form-group">
                <label class="control-label text-bold" for="social_site_link">Social Site Link</label>
                <input type="url" name="social_site_link" id="social_site_link" class="form-control" data-msg-required="Please enter social site link." value="{{$event->social_site_link ?? ''}}">
                <span class="validation-errors"></span>
            </div>

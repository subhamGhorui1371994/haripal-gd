...
@section('content')
    ...
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title text-bold">Edit Session Students</h3>
            <div class="heading-elements">
                <a href="{{ url('admin/students') }}" class="btn btn-primary">List Session Students</a>
            </div>
        </div>
        <div class="panel-body">
            {{ Form::open(array('method' => 'PATCH','route' => ['students.update',$students->id], 'class'=>'form-vertical', 'id' => 'portfolio_form', 'files' => true )) }}

            <div class="form-group">
                <label class="control-label text-bold" for="course_type">Course <span class="required-fields">*</span></label>
                {!! Form::select('course_type',[''=>'Select Course', 'B.ED.' => 'B.ED.', 'D.EL.ED.'=> 'D.EL.ED.'], ($students->course_type ?? ''),['id'=>'course_type', 'class'=>'form-control select-2', 'required'=>'required', 'data-msg-required'=>'Please select a course.'])!!}
            ...
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" name="session_start" id="session_start" class="form-control" required data-msg-required="Please enter session start year." value="{{$students->session_start ?? ''}}">
                    <span class="validation-errors"></span>
                </div>

                <div class="form-group col-md-6">
                    <label class="control-label text-bold" for="session_end">Session End <span class="required-fields">*</span></label>
                    <input type="text" name="session_end" id="session_end" class="form-control" required data-msg-required="Please enter session end year." value="{{$students->session_end ?? ''}}">
                    <span class="validation-errors"></span>
                </div>
            </div>

            @if($students->student_data)
                <div class="form-group">
                    <label class="control-label text-bold" for="logo">Students File</label>
                    <input type="file" name="student_data" id="student_data" class="form-control" data-msg-required="Please select a students file.">
                    <span class="validation-errors"></span>
                </div>
                <div class="form-group">
                    <a href="{{ URL::asset($students->student_data) }}" class="btn btn-sm btn-outline-warning"><i class="icon-download"></i> Check the uploaded file.</a>
                </div>
            @else
                <div class="form-group">
